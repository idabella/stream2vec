...
@pytest.mark.asyncio
async def test_end_to_end_ingestion_and_search(integration_client: AsyncClient):
    ...
    # 1. Upload
    upload_res = await integration_client.post("/api/v1/documents", files=files)
    assert upload_res.status_code == 201
    doc_id = upload_res.json()["id"]

    # 2. Check document initially recorded
    get_res = await integration_client.get(f"/api/v1/documents/{doc_id}")
    ...
    search_res = await integration_client.post("/api/v1/search", json=search_payload)
